...
FIRST_DAY_OF_SCHOOL = 'YYYY-MM-DD'
...
def delete_rows_after_firstday_from(tablename):
	t = declare_database_table(tablename)
	if tablename == 'Canvas_Submissions':
		CONNECTION.execute(t.delete().where(t.c.submitted_at > FIRST_DAY_OF_SCHOOL))
	elif tablename == 'Canvas_PageViews':
		CONNECTION.execute(t.delete().where(t.c.datetime >= FIRST_DAY_OF_SCHOOL))
...
def get_submissions(headers):
	...
	for section in ACTIVE_SECTIONS:
		...
		params = {'submitted_since' : FIRST_DAY_OF_SCHOOL + 'T00:00:00Z'}
			...
			for user in course_users:
				...
				if response:
					for key, value in response['page_views'].items():
						if datetime.strptime(key[:10], '%Y-%m-%d') >= datetime.strptime(FIRST_DAY_OF_SCHOOL, '%Y-%m-%d'):
...
try:
	...
	delete_rows_after_firstday_from('Canvas_Submissions')
	delete_rows_after_firstday_from('Canvas_PageViews')
